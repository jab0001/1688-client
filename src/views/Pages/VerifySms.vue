<template>
  <FormVerifySms @onSubmit="onSubmitHandler" />
</template>
<script>
  ...
  methods: {
    ...
    async onSubmitHandler(payload) {
      ...
      try {
        ...
        if (res.data.token && verifyFor === 'signin') {
          this.$router.push('/');
        }

        if (res.status === 200 && verifyFor === 'signup') {
          this.$router.push({
            name: 'SuccessVerifySms',
            params: { verify: verifyFor },
          });
        }
        ...
        if (res.status === 200 && verifyFor === 'forgot') {
          this.$router.push({
            name: 'Reset',
            params: { phone: payload.phone, code },
          });
        }
      } catch (e) {
        console.log(e);
